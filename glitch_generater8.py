...
def apply_skin_noise(roi, mask, noise_mode):
    ...
    if noise_mode == 'grain':
        ...
        # 粒を大きくするため、一度小さなサイズでノイズを作って拡大する
        # scaleが小さいほど、拡大率が上がり粒が大きくなる (例: 0.2 = 5倍拡大)
        ...
        # ブロックを大きくする
        scale = random.uniform(0.05, 0.25) # かなり粗くする
        small_h, small_w = int(h * scale), int(w * scale)
        if small_h < 1: small_h = 1
        if small_w < 1: small_w = 1
        ...
        for filename in files:
            ...
            for i, face_landmarks in enumerate(results.multi_face_landmarks):
                ...
                for mode in noise_modes:
                    ...
                    # ノイズ適用（修正済み関数）
                    noisy_roi = apply_skin_noise(roi, final_mask_normalized, mode)
                    
                    output_img[y1:y2, x1:x2] = noisy_roi
                    
                    base_name = os.path.splitext(filename)[0]
                    save_base = f"{base_name}_{mode}_{i}"
